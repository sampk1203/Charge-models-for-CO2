...
fraction_autocorrelation_function_to_fft = 0.7 # since you only want to fft the part that has meaningful statistics,
...
if autocorrelation_option == 1:
    # Load data
    time, dipole_x, dipole_y, dipole_z = np.loadtxt(input_dipole_file, skiprows=2, usecols=(0,1,2,3), unpack=True)
...
timestep = (time[1]-time[0]) * 1.E-12 # converts time from femtoseconds to seconds
...
quantum_correction = lineshape_frequencies ** 2
#quantum_correction = lineshape_frequencies ** 2
